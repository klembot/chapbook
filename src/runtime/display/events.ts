import {SystemThemeChangeEventDetail} from '../custom-events';
...
let themeWatcher: MediaQueryList;

/**
 * Sets up listeners that emit:
 * - `display-change` events with rendered content when the trail state variable
 *   changes.
 * - `system-theme-change` events when the system theme changes.
 */
export function initDisplayEvents() {
  ...
  themeWatcher.addEventListener('change', () => {
    window.dispatchEvent(
      new CustomEvent<SystemThemeChangeEventDetail>('system-theme-change', {
        detail: {theme: themeWatcher.matches ? 'dark' : 'light'}
      })
    );
  });
}
